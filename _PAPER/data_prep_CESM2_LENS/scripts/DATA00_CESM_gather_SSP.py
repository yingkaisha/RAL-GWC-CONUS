...
def select_fn_fmt(fn_fmt_list, year):
    ...
    for i, s in enumerate(fn_fmt_list):
        m = re.search(r'(\d{4})\d{6}-(\d{4})\d{6}\.nc$', s)
        if not m:
            continue
...
fn_fmt_list = [
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h5.{}.2065010100-2074123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h5.{}.2075010100-2084123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h5.{}.2085010100-2094123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h5.{}.2095010100-2100123100.nc']

ind_year, fn_fmt = select_fn_fmt(fn_fmt_list, year)

dict_fn = {}

for varname in varnames:
    fn_ = fn_fmt.format(ens, varname)
    dict_fn[varname] = os.path.join(base_dir, varname, fn_)
    
varnames_surf = ['TMQ', 'PS', 'PSL', 'UBOT', 'VBOT']

fn_fmt_surf = [
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h2.{}.2065010100-2074123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h2.{}.2075010100-2084123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h2.{}.2085010100-2094123100.nc',
    'b.e21.BSSP370smbb.f09_g17.LE2-{}.cam.h2.{}.2095010100-2100123100.nc']
...
ds = xr.merge(ds_collcetion_upper + ds_collcetion_surf)
...
save_name = f'/glade/derecho/scratch/ksha/DWC_data/CONUS_domain_GP/dscale_CESM_SSP/CESM_GP_{year}.zarr'
