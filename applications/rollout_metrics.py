...
def predict(rank, world_size, conf, backend=None, p=None):
    ...
    if len(forecasts) % world_size != 0:
        raise ValueError(f'Number of forecast inits ({len(forecasts)}) given by conf["predict"]["duration"] x len(conf["predict"]["start_hours"]) should be divisible by number of processes/GPUs ({world_size})')
