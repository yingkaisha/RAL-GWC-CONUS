...
def load_state_dict(checkpoint_file_path: Path):
    ...
    assert not is_dtensor_checkpoint(checkpoint_file_path), f"Cannot load state dict from dtensor checkpoint {checkpoint_file_path}, you should convert the distributed tensors to gathered tensors with our CLI offline."
